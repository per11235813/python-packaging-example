...
def update():
    ...
    pyproject_toml = tomli.loads(Path("pyproject.toml").read_text(encoding="utf8"))
    dependencies = pyproject_toml["project"]["dependencies"]
    dev_dependencies = pyproject_toml["project"]["optional-dependencies"]["dev"]
    update_str = " ".join(dependencies + dev_dependencies)
    run(fr".\venv\Scripts\activate.bat & python -m pip install -U {update_str}")
...
def check_git() -> tuple[str, str, str]:
    ...
    if is_git_tag_used(git_tag):
        print(f"Git tag: {git_tag} is already used. Update version in pyproject.toml. Exiting ...", file=sys.stderr)
        sys.exit(1)
